...
export class FileManagerService {
 ...
 Upload(model: Filemanager,name): Observable<Filemanager> {
        ...
        return this.http.post<Filemanager>(`http://localhost:8082/curis/qms_file/import/${name}`,model)
        . pipe(
  ...
  Run(selectedModel): Observable<Filemanager>{ 
    return this.http.get<Filemanager>(`http://localhost:8082/curis/qms_file/run_r/${selectedModel}`)
        . pipe(
