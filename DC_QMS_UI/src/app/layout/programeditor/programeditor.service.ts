...
export class ProgrameditorService {
  ...
  programCreatorSubmit(model: ProgramCreator): Observable<ProgramCreator> {
        ...
        return this.http.post<ProgramCreator>('http://localhost:8082/curis/program/editProgram', model, httpOptions)
        . pipe(
  ...
  getProgramNames(){
    return this.http.get(`http://localhost:8082/curis/qms/dropdown_list/QMS_QUALITY_PROGRAM/PROGRAM_NAME`);
  }
  getProgramDetails(name){
    return this.http.get(`http://localhost:8082/curis/program/getProgramByName/${name}`)
  }
