...
export class MeasurecreatorService {
  ...
  MeasurecreatorSubmit(model: any): Observable<any> {
        ...
        return this.http.post('http://localhost:8082/curis/qms/createProgram', model, httpOptions)
        . pipe(
  ...
  getMeasuerInfo(measureId) {
    return this.http.get(`http://localhost:8082/curis/qms/measure_list/${measureId}`);
  }
}
