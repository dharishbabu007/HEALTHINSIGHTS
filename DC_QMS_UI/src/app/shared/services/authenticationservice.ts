...
export class AuthenticationService {
    ...
    login(username: string, password: string) {
        return this.http.get<any>(`http://localhost:8082/curis/user/login/${username}/${password}`)
            .pipe(
    ...
    Reset(loginId,oldPassword,newPassword,confirmPassword){
        ...
        return this.http.post<any>('http://localhost:8082/curis/user/reset_password/',{
             "userId":loginId,
      ...
      Register(model){
          return this.http.post<any>('http://localhost:8082/curis/user/create_user/',{
            "name": model.name,
            "email": model.email,
            "loginId": model.name,
