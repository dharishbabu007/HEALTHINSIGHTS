...
export class PatScreenService {
  ...
  commonfileupload(type,model: PatScreen): Observable<PatScreen> {
    return this.http.post<PatScreen>(`http://healthinsight:8082/curis/closeGaps/file_upload/${type}`,model) . pipe(catchError((error, caught) => {
  ...
  getCodeType(measureId,valueset){
    return this.http.get(`http://healthinsight:8082/curis/pat/action_on_caregap_Code_Type_list/${measureId}/${valueset}`)
  }
  getCodeList(measureId,valueset,codeType){
    return this.http.get(`http://healthinsight:8082/curis/pat/action_on_caregap_Codes_list/${measureId}/${valueset}/${codeType}`)
  }
}
