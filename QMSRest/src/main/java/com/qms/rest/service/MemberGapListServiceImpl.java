...
//					"GROUP BY DM.MEMBER_ID, CONCAT(DM.FIRST_NAME,' ',DM.LAST_NAME), DM.GENDER, DD.DATE_SK, DD.CALENDAR_DATE, GIC1.QUALITY_MEASURE_ID, QM.MEASURE_TITLE";
			...
			*/
			...
			+"INNER JOIN QMS.QMS_GIC_LIFECYCLE GIC1 ON GIC1.MEMBER_ID = DM.MEMBER_ID " 
			+"LEFT OUTER JOIN QMS.QMS_GIC_LIFECYCLE GIC2 ON GIC1.QUALITY_MEASURE_ID = GIC2.QUALITY_MEASURE_ID AND GIC1.MEMBER_ID = GIC2.MEMBER_ID " 
			+"INNER JOIN QMS.DIM_QUALITY_MEASURE QM ON GIC1.QUALITY_MEASURE_ID=QM.QUALITY_MEASURE_ID " 
			+"INNER JOIN QMS.DIM_DATE DD ON DD.DATE_SK = DM.DATE_OF_BIRTH_SK " 
			+"WHERE DM.MEMBER_ID="+mid+" " 
...
//					"DM.GENDER, CONCAT(DP.FIRST_NAME,' ',DP.LAST_NAME), DQM.MEASURE_TITLE, GIC.STATUS, DPP.PLAN_NAME, GIC.GAP_DATE";
			...
			+"FROM QMS.QMS_GIC_LIFECYCLE GIC "
			+"INNER JOIN QMS.DIM_MEMBER DM ON DM.MEMBER_ID = GIC.MEMBER_ID "
			+"INNER JOIN QMS.FACT_MEM_ATTRIBUTION FMA ON FMA.MEMBER_SK = DM.MEMBER_SK "
			+"INNER JOIN QMS.DIM_PROVIDER DP ON DP.PROVIDER_SK = FMA.PROVIDER_SK "
			+"INNER JOIN QMS.DIM_QUALITY_MEASURE DQM ON DQM.QUALITY_MEASURE_ID = GIC.QUALITY_MEASURE_ID "
			+"INNER JOIN QMS.DIM_PRODUCT_PLAN DPP ON DPP.PRODUCT_PLAN_ID = GIC.PRODUCT_PLAN_ID "
