...
//			statement.executeQuery("ALTER TABLE NS_FILE_INPUT ADD PARTITION (fid="+fileId+") LOCATION '"+hdfsInputLocation+"'");
	...
	@Override
	public Set<CSVOutPut> getComplianceOutPut() {
		
		Set<CSVOutPut> setOutput = new HashSet<>();
		
	    BufferedReader br = null;
		try {		
			br = new BufferedReader(new FileReader(windowsCopyPath+"/compliance/Output.csv"));			
		    int i = 0;
		    String line = null;
		    CSVOutPut output = null;
		    while ((line = br.readLine()) != null) {
	...
	@Override
	public Set<ConfusionMatric> getComplianceConfusionMatric() {
		Set<ConfusionMatric> setOutput = new HashSet<>();
		
	    BufferedReader br = null;
		try {
			br = new BufferedReader(new FileReader(windowsCopyPath+"/compliance/ConfusionMatric.csv"));
		    String line = null;
		    ConfusionMatric output = null;
		    int i = 0;
		    while ((line = br.readLine()) != null) {
	...
	@Override
	public ModelScore getComplianceModelScore() {
		ModelScore output = new ModelScore();
		
	    BufferedReader br = null;
		try {
			br = new BufferedReader(new FileReader(windowsCopyPath+"/compliance/ModelScore.csv"));
		    String line = null;
		    while ((line = br.readLine()) != null) {
		    	if(line != null && !line.trim().isEmpty()) {
			    	String[] values = line.split(",");
			    	output = new ModelScore();	
			    	output.setScore(values[0]);
			    	output.setImageFullPath(windowsCopyPath+"/compliance/ROCplot.PNG");
		    	}
		    }		    
		} catch (Exception e) {
			e.printStackTrace();
