...
public class CloseGapsServiceImpl implements CloseGapsService {
	...
	public CloseGaps getCloseGaps(String memberId, String measureId) {
		...
		try {						
			...
			resultSet = statement.executeQuery("select dm.gender, dm.member_id, (dm.FIRST_NAME||' '||dm.MIDDLE_NAME||' '||dm.LAST_NAME) AS NAME, "
					+ "dd.CALENDAR_DATE from QMS.dim_member dm, QMS.dim_date dd "
			...
			if (resultSet.next()) {
				...
				closeGaps.setMemberId(resultSet.getInt("dm.member_id")+"");
			...
			String membergplistQry = "SELECT MIN(GIC.GAP_DATE) AS OPENDATE,DQM.MEASURE_TITLE,GIC.TARGET_DATE,GIC.STATUS,GIC.GAP_DATE,"+
					"(DP.FIRST_NAME||' '||DP.LAST_NAME) AS PCP FROM QMS.DIM_QUALITY_MEASURE DQM "+
					"INNER JOIN QMS.QMS_GIC_LIFECYCLE GIC ON GIC.QUALITY_MEASURE_ID = DQM.QUALITY_MEASURE_ID "+
					"INNER JOIN QMS.DIM_MEMBER ENC ON ENC.MEMBER_ID = GIC.MEMBER_ID "+
					"INNER JOIN QMS.FACT_MEM_ATTRIBUTION FMA ON FMA.MEMBER_SK = ENC.MEMBER_SK "+ 
					"INNER JOIN QMS.DIM_PROVIDER DP ON DP.PROVIDER_SK = FMA.PROVIDER_SK "+
			...
			//measure details			
			if (resultSet.next()) {
				closeGaps.setCareGap(resultSet.getString("DQM.measure_title")); 
				closeGaps.setOpenDate(resultSet.getString("OPENDATE"));
				closeGaps.setTargetDate(resultSet.getString("GIC.TARGET_DATE"));
				closeGaps.setAssignedTo(resultSet.getString("PCP")); 
				closeGaps.setStatus(resultSet.getString("GIC.STATUS")); 
				closeGaps.setLastActionDate(resultSet.getString("GIC.GAP_DATE"));
			}
			...
			while (resultSet.next()) {
				...
				closeGap.setLifeCycleId(resultSet.getInt("qgl.GIC_LIFECYCLE_ID"));
				...
				String dateStr = QMSDateUtil.getSQLDateFormat(resultSet.getDate("qgl.TARGET_DATE"));
	...
	public RestResult insertCloseGaps(CloseGaps closeGaps, String memberId, String measureId) {
		...
		String sqlStatementInsert = "UPSERT into QMS.qms_gic_lifecycle (member_id,quality_measure_id,interventions,"
		...
		try {	
			...
			resultSet = statementObj.executeQuery("select gap_date,status,HEDIS_GAPS_IN_CARE_SK,PRODUCT_PLAN_ID,user_name "
					+ "from QMS.qms_gic_lifecycle where member_id="+memberId+" and "
			...
			resultSet.close();
			int lifeCycleId = 0;
			resultSet = statementObj.executeQuery("select max(gic_lifecycle_id) from QMS.qms_gic_lifecycle");
			while (resultSet.next()) {
			...
			int i=0;							
			statement.setInt(++i, Integer.parseInt(memberId));
			statement.setInt(++i, Integer.parseInt(measureId));
			...
			statement.setInt(++i, Integer.parseInt(productId));
	...
	public RestResult saveFileUpload(GicLifeCycleFileUpload fileUpload) {		
		...
		try {
			...
			resultSet = statementObj.executeQuery("select max(FILE_ID) from QMS.QMS_FILE_UPLOAD");
			int fileId = 0;
			while (resultSet.next()) {
			...
			"UPSERT into QMS.QMS_FILE_UPLOAD(FILE_ID,TYPE_ID,PATH,FILE_NAME,TYPE,"+
	...
	public HashMap<Integer, List<String>> getUploadFileByTypeId (Statement statement, List<Integer> typeIds, 
		...
		ResultSet resultSet = statement.executeQuery("select TYPE_ID,PATH,FILE_NAME from "
				+ "QMS.QMS_FILE_UPLOAD where TYPE='"+type+"' AND TYPE_ID in('"+glcIds+"')");
		int typeIdLocal = 0;
		List<String> fileNames = null;
		while (resultSet.next()) {
