            ...
            app.controller("QMSHomeController", function($scope, $rootScope, $http) {
                ...
                function _listReimbursementPrograms() {
                    $http({
                        method : 'GET',
                        //url : 'http://localhost:8082/curis/qms/dropdown_list/dim_quality_measure_main/program_name'
						url : 'http://localhost:8082/curis/qms/qmshome_dropdown_list/QMS_MEASURE/PROGRAM_NAME'
                ...
                function _listClinicalConditions() {
                    $http({
                        method : 'GET',
                        //url : 'http://localhost:8082/curis/qms/dropdown_list/dim_quality_measure_main/clinical_conditions'
						url : 'http://localhost:8082/curis/qms/qmshome_dropdown_list/QMS_MEASURE/CLINICAL_CONDITIONS'
                    }).then(function successCallback(response) {
                ...
                function _listNQFDomains() {
                    $http({
                        method : 'GET',
                        //url : 'http://localhost:8082/curis/qms/dropdown_list/dim_quality_measure_main/nqs_domain'
						url : 'http://localhost:8082/curis/qms/qmshome_dropdown_list/QMS_MEASURE/DOMAIN'
                    }).then(function successCallback(response) {
            ...
            app.controller("MeasureListController", function($scope, $rootScope, $http, $location, $window) {
                ...
                function _listMeasures() {
                    $http({
                        method : 'GET',
                        url : 'http://localhost:8082/curis/qms/measure_list/'+filterType+'/'+filterValue
                    }).then(function successCallback(response) {
            ...
            app.controller("WorklistController", function($scope, $rootScope, $http, $location, $window) {					
                ...
                function _listWorklist() {
                    $http({
                        method : 'GET',
                        url : 'http://localhost:8082/curis/qms/work_list/'
                    }).then(function successCallback(response) {
				...
				$scope.statusClick = function (status) {					
					$http.put('http://localhost:8082/curis/qms/work_list/status/'+$scope.selectedId+'/'+status)		
					.then(function successCallback(response) {
				...
				$scope.measureConfigClick = function () {
					...
					$http({
						method : 'GET',
						url : 'http://localhost:8082/curis/measure_configurator/config_data/qms_input/qms_input123'
					}).then(function successCallback(response) {
	...
	app.controller("MeasureCreateController", function($scope, $rootScope, $http, $location) {
		...
		function _listMeasureProgramNames() {
			$http({
				method : 'GET',				
				url : 'http://localhost:8082/curis/qms/dropdown_namevalue_list/qms_quality_program/PROGRAM_ID/PROGRAM_NAME'
			}).then(function successCallback(response) {
			...
			$http({
				method : 'GET',				
				url : 'http://localhost:8082/curis/qms/dropdown_namevalue_list/qms_quality_program/PROGRAM_NAME/CATEGORY_NAME'
			}).then(function successCallback(response) {
		...
		function _listMeasureTypes() {
			$http({
				method : 'GET',
				url : 'http://localhost:8082/curis/qms/dropdown_namevalue_list/QMS_MEASURE_TYPE/MEASURE_TYPE_ID/MEASURE_TYPE_NAME'
			}).then(function successCallback(response) {
		...
		function _getMeasureLib() {
			$http({
				method : 'GET',
				url : 'http://localhost:8082/curis/qms/measure_list/'+selectedMeasureId
			}).then(function successCallback(response) {
		...
		$scope.submitAdd = function (action) {	
			...
			$http.post('http://localhost:8082/curis/qms/work_list/', $scope.measureForm)		
			.then(function successCallback(response) {
				$scope.restResult = response.data;
				console.log(response.statusText);
	...
	app.controller("MeasureEditController", function($scope, $rootScope, $http, $location) {					
		...
		function _getMeasureLib() {
			if(source == "measureLibrary") {
				...
				$http({
					method : 'GET',
					url : 'http://localhost:8082/curis/qms/work_list/'+selectedMeasureId
				}).then(function successCallback(response) {
					$scope.measureForm = response.data;
					console.log(" measure --> " + $scope.measureForm);
				}, function errorCallback(response) {
		...
		$scope.submitEdit = function (action) {			
			...
			$http.put('http://localhost:8082/curis/qms/work_list/'+selectedMeasureId, 
			$scope.measureForm)		
			.then(function successCallback(response) {
				$scope.restResult = response.data;				
	...
	app.controller("IndexDotHTMLController", function($scope, $rootScope, $http, $location, $window) {		
		...
		$scope.viewPatientProfile = function (patienceId) {
			...
			$http({
				method : 'GET',
				//url : 'http://localhost:8082/curis/qms/spv/'+patienceId
				url : 'http://localhost:8082/curis/qms/spv/2014875'
			}).then(function successCallback(response) {
	...
	app.controller("LoginController", function($scope, $rootScope, $http, $location, $window) {
		...
		$scope.submit = function () {
			...
			$http({
				method : 'GET',
				url : 'http://localhost:8082/curis/qms/user/'+$scope.userName+'/'+$scope.password
			}).then(function successCallback(response) {
...
app.controller('QueryBuilderCtrl', ['$scope', '$rootScope', '$http', '$window', function ($scope, $rootScope, $http, $window) {
    ...
    function computed(group) {			
		...
		$scope.categoryClick = function (configType) {
			...
			$http({
				method : 'GET',
				url : 'http://localhost:8082/curis/measure_configurator/'+selectedMeasureId+'/'+configType
			}).then(function successCallback(response) {
	...
	$scope.submit = function (action) {
		...
		$scope.restResult = {status : "", meaasge : ""};
		$http.post('http://localhost:8082/curis/measure_configurator/'+selectedMeasureId+'/'+$scope.configType, 
		$scope.group.rules)		
		.then(function successCallback(response) {
			$scope.restResult = response.data;				
			console.log(response.data);			
