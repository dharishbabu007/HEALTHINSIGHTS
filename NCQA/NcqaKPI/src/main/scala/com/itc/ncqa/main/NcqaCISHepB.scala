...
object NcqaCISHepB {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQAHEPB")
    ...
    /*Numerator1 Calculation (Hepb screening or monitoring test)*/

    val hedisJoinedForHepbProcCodeScreeningDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cisHepbMeasureId, KpiConstants.cisHepbValueSet, KpiConstants.cisHepbCodeSystem)
    val hedisJoinedForHepbPrimaryScreeningDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.cisHepbMeasureId, KpiConstants.cisHepbValueSet, KpiConstants.primaryDiagnosisCodeSystem)

    val hedisJoinedForHepbScreeningDf = hedisJoinedForHepbProcCodeScreeningDf.union(hedisJoinedForHepbPrimaryScreeningDf)

    val measurement = UtilFunctions.mesurementYearFilter(hedisJoinedForHepbScreeningDf, KpiConstants.startDateColName, year, KpiConstants.measurementYearLower, KpiConstants.measuremetTwoYearUpper)
    ...
    val dayFilterConditionNumeratorDf = ageFilterJoinNumeratorDf.filter(datediff(date_add(ageFilterJoinNumeratorDf.col("dob"),730),ageFilterJoinNumeratorDf.col("start_date")).>=(0))

    val cisHepbJoinDf = dayFilterConditionNumeratorDf.as("df1").join(factClaimDf.as("df2"), $"df1.member_sk" === $"df2.member_sk", "inner").select("df2.member_sk", "df2.start_date_sk")

    /* Filtering members with At least three Hepb vaccinations */

    val cisHepbCountDf = cisHepbJoinDf.groupBy("member_sk").agg(count("start_date_sk").alias("count1")).filter($"count1".>=(3)).select("df2.member_sk")

    val cisHepbNumeratorDf = cisHepbCountDf.intersect(dinominatorAfterExclusionDf)


    /*common output creation(data to fact_gaps_in_hedis table)*/
    val numeratorReasonValueSet = KpiConstants.cisHepbValueSet
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.cisHepbMeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinoExclDf, cisHepbNumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
