...
object NcqaCDC4 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //<editor-fold desc="Numerator First Condition(Diabetic Retinal Screening)">

    val diabRetScreenValList = List(KpiConstants.diabeticRetinalScreeningVal)
    val diabRetScreenCodeSystem = List(KpiConstants.cptCodeVal, KpiConstants.hcpsCodeVal)
    val joinedForretScreenDf = UtilFunctions.factClaimRefHedisJoinFunction(spark, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cdcMeasureId, diabRetScreenValList, diabRetScreenCodeSystem)
    ...
    val joinedWithDimProviderDf = joinedForretScreenDf.as("df1").join(dimProviderDf.as("df2"), joinedForretScreenDf.col(KpiConstants.providerSkColName) === dimProviderDf.col(KpiConstants.providerSkColName), KpiConstants.innerJoinType)
                                                                       .filter(dimProviderDf.col(KpiConstants.eyeCareProvColName).===(KpiConstants.yesVal)).select("df1.*")
    val measurForRetScreenDf = UtilFunctions.mesurementYearFilter(joinedWithDimProviderDf, KpiConstants.startDateColName, year, KpiConstants.measurement0Val, KpiConstants.measurement1Val).select(KpiConstants.memberskColName)
    //</editor-fold>

    //<editor-fold desc="Numerator Second Condition(negative Retinopathy during year prior to Mesurement),Not completed">

    val measureFornegRetino = UtilFunctions.measurementYearFilter(joinedWithDimProviderDf, KpiConstants.startDateColName, year, KpiConstants.measurement1Val, KpiConstants.measurement2Val).select(KpiConstants.memberskColName)
    //</editor-fold>

    //<editor-fold desc="Numerator Third Condition(Diabetic Retinal And Diabetic diagnosis without complication)">

    val diabtesMelWoComplValList = List(KpiConstants.diabtesMellWoCompliVal)
    val diabtesMelWoComplCodeSystem = List(KpiConstants.cptCodeVal, KpiConstants.hcpsCodeVal)
    val joinedFordiabtesMelWoComplDf = UtilFunctions.factClaimRefHedisJoinFunction(spark, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cdcMeasureId, diabtesMelWoComplValList, diabtesMelWoComplCodeSystem)
    val measrFordiabtesMelWoComplDf = UtilFunctions.measurementYearFilter(joinedFordiabtesMelWoComplDf, KpiConstants.startDateColName, year, KpiConstants.measurement1Val, KpiConstants.measurement2Val).select(KpiConstants.memberskColName)
    val drAnddmwcDf = measureFornegRetino.intersect(measrFordiabtesMelWoComplDf)
    //</editor-fold>

    //<editor-fold desc="Numerator Fourth Condition(Diabetic Retinal Screening with Eye care professional)">

    val diabeticRetScrewecprofValList = List(KpiConstants.diabeticReScreeWECProfessionalVal)
    val diabeticRetScrewecprofCodeSystem = List(KpiConstants.cptCodeVal, KpiConstants.hcpsCodeVal)
    val joinedForEyeCareProfDf = UtilFunctions.factClaimRefHedisJoinFunction(spark, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cdcMeasureId, diabeticRetScrewecprofValList, diabeticRetScrewecprofCodeSystem)
    val measurForEyeCareProfDf = UtilFunctions.measurementYearFilter(joinedForEyeCareProfDf, KpiConstants.startDateColName, year, KpiConstants.measurement0Val, KpiConstants.measurement1Val).select(KpiConstants.memberskColName)
    //</editor-fold>

    //<editor-fold desc="Numerator Fifth Condition(Diabetic Retinal Screening with Eye care professional with negative retinopathy) not completed">

    val measurementForFifthNumeratorDf = UtilFunctions.measurementYearFilter(joinedForEyeCareProfDf, KpiConstants.startDateColName, year, KpiConstants.measurement1Val, KpiConstants.measurement2Val).select(KpiConstants.memberskColName)
    //</editor-fold>

    //<editor-fold desc="Numerator Sixth Condition(Diabetic rEtinal with Negative)">

    val diabeticRenScreNegativeValList = List(KpiConstants.diabeticRetinalScreeningVal)
    val diabeticRenScreNegativeCodeSystem = List(KpiConstants.cptCodeVal, KpiConstants.hcpsCodeVal)
    val joinedForDiabeticretiScrnDf = UtilFunctions.factClaimRefHedisJoinFunction(spark, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cdcMeasureId, diabeticRenScreNegativeValList, diabeticRenScreNegativeCodeSystem)
    val measurForDiabeticretiScrnDf = UtilFunctions.measurementYearFilter(joinedForDiabeticretiScrnDf, KpiConstants.startDateColName, year, KpiConstants.measurement0Val, KpiConstants.measurement1Val).select(KpiConstants.memberskColName)
    //</editor-fold>

    //<editor-fold desc="Numerator Seventh Condition(Unilateral and Bilateral)">

    val unilateralEyeEnucValList = List(KpiConstants.unilateralEyeEnucleationVal)
    val unilateralEyeEnucCodeSystem = List(KpiConstants.cptCodeVal, KpiConstants.hcpsCodeVal)
    val joinedForUnilateralEyeUnicDf = UtilFunctions.factClaimRefHedisJoinFunction(spark, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cdcMeasureId, unilateralEyeEnucValList, unilateralEyeEnucCodeSystem).select(KpiConstants.memberskColName)
    val bilateralValList = List(KpiConstants.bilateralModVal)
    val bilateralCodeSystem = List(KpiConstants.cptCodeVal, KpiConstants.hcpsCodeVal)
    val joinedForBilateralDf = UtilFunctions.factClaimRefHedisJoinFunction(spark, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cdcMeasureId, bilateralValList, bilateralCodeSystem).select(KpiConstants.memberskColName)
    val unilatAndbilatDf = joinedForUnilateralEyeUnicDf.intersect(joinedForBilateralDf)
    //</editor-fold>

    //<editor-fold desc="Numerator Eight(Unilateral Left with 14 days apart)">

    val unilateyeenuLeftValList = List(KpiConstants.unilateralEyeEnuLeftVal)
    val unilateyeenuLeftCodeSystem = List(KpiConstants.cptCodeVal, KpiConstants.hcpsCodeVal)
    val joinedForUnilateralEyeEnuLeftDf = UtilFunctions.factClaimRefHedisJoinFunction(spark, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cdcMeasureId, unilateyeenuLeftValList,unilateyeenuLeftCodeSystem)
    val selfJoin = joinedForUnilateralEyeEnuLeftDf.as("df1").join(joinedForUnilateralEyeEnuLeftDf.as("df2"), $"df1.member_sk" === $"df2.member_sk", "inner").select("df1.member_sk", "df1.start_date", "df2.start_date")
    //val selectedColumnDf = selfJoin.select("df1.member_sk","df1.start_date","df2.start_date").withColumn("second_date",$"df2.start_date")
    val unilatenuLeftDf = selfJoin.filter(datediff(selfJoin.col("df2.start_date"), selfJoin.col("df1.start_date")).>=(14)).select("member_sk")
    //</editor-fold>

    //<editor-fold desc="Numerator Nine(unilateral left and right on same or diffrenet dates)">

    val unilatEyeEnuRightValList = List(KpiConstants.unilateralEyeEnuRightVal)
    val unilatEyeEnuRightCodeSystem = List(KpiConstants.cptCodeVal, KpiConstants.hcpsCodeVal)
    val joinedUnilateralEyeEnuRightDf = UtilFunctions.factClaimRefHedisJoinFunction(spark, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cdcMeasureId, unilatEyeEnuRightValList, unilatEyeEnuRightCodeSystem)
    val unileftAndRightDf = joinedForUnilateralEyeEnuLeftDf.as("df1").join(joinedUnilateralEyeEnuRightDf.as("df2"), $"df1.member_sk" === $"df2.member_sk", KpiConstants.innerJoinType).select("df1.member_sk")
    //</editor-fold>


    val unionOfAllNumeratorDf = measurForRetScreenDf.union(measureFornegRetino).union(drAnddmwcDf).union(measurForEyeCareProfDf).union(measurementForFifthNumeratorDf).union(measurForDiabeticretiScrnDf).union(unilatAndbilatDf).union(unilatenuLeftDf).union(unileftAndRightDf)
    val numeratorDf = unionOfAllNumeratorDf.intersect(dinominatorAfterExclDf)
    //</editor-fold>
