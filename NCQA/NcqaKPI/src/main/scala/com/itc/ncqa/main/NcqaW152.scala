...
object NcqaW152 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQAW152")
    ...
    val hedisJoinedForWellCareAsPrDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.w152MeasureId, KpiConstants.w15ValueSetForNumerator, KpiConstants.w15CodeSystemForNum)
    ...
    val hedisJoinedForWellCareAsDiagDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.w152MeasureId, KpiConstants.w15ValueSetForNumerator, KpiConstants.primaryDiagnosisCodeSystem)
    ...
    val w152NumeratorGeneral = measurementForWellCareAsPrDf.union(measurementForWellCareAsDiagDf)

    /*Start_date_sk for members who have pcp equal to 'Y'*/


    val w152WellCareJoinDf = dimProviderDf.as("df1").join(factClaimDf.as("df2"), ($"df1.provider_sk" === $"df2.provider_sk"), "inner").filter(($"df1.pcp" === KpiConstants.yesVal)).select("df2.member_sk", "df2.start_date_sk").distinct()

    val w150JoinNumerator = w152NumeratorGeneral.as("df1").join(w152WellCareJoinDf.as("df2"), ($"df1.member_sk" === $"df2.member_sk"), joinType = "inner").select("df2.member_sk", "df2.start_date_sk")

    val w152WellCareCountZeroDf = w150JoinNumerator.groupBy("member_sk").agg(count("start_date_sk").alias("count1")).filter($"count1".===(2)).select(KpiConstants.memberskColName)

    val w152NumeratorDf = w152WellCareCountZeroDf.intersect(dinominatorAfterExclusionDf)
    w152NumeratorDf.show()
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.w152MeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinominatorAfterExclusionDf, w152NumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
