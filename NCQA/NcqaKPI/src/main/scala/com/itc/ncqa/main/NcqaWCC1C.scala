...
object NcqaWCC1C {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NcqaWCC1C")
    ...
    val hedisJoinedForDinominator = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.wcc1cMeasureId, KpiConstants.abavalueSetForDinominator, KpiConstants.abscodeSystemForDinominator)
    ...
    val joinForDinominatorExcl = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.wcc1cMeasureId, KpiConstants.abavaluesetForDinExcl, KpiConstants.primaryDiagnosisCodeSystem)
    ...
    val joinForNumeratorForPhysical1 = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.wcc1cMeasureId, KpiConstants.wcc1cPhysicalValueSet, KpiConstants.primaryDiagnosisCodeSystem)
    ...
    /*Numerator1 Calculation (Physical Activity  Value Set ) for primaryDiagnosisCodeSystem */
    val joinForNumeratorForPhysical2 = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.wcc1cMeasureId, KpiConstants.wcc1cPhysicalValueSet, KpiConstants.wcc1cPhysicalCodeSystem)
    ...
    val wcc1cnumeratorFinalDf = numeratorPhysicalDf.intersect(finalDinominatorDf).select(KpiConstants.memberskColName).distinct()

    wcc1cnumeratorFinalDf.show()
    ...
    val sourceAndMsrIdList = List(data_source,KpiConstants.wcc1cMeasureId)
    val numExclDf = spark.emptyDataFrame
    val outFormatDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorForOutput, dinominatorExcl, wcc1cnumeratorFinalDf, numExclDf, listForOutput, sourceAndMsrIdList)
