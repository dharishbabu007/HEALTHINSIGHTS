...
object NcqaIMAHPV {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQAIMAHPV")
    ...
    /*filter out the members whoose age is between 9 and 13 */
<<<<<<< HEAD
    val ageFilterDf = UtilFunctions.ageFilter(commonFilterDf, KpiConstants.dobColName, year, KpiConstants.age9Val, KpiConstants.age13Val, KpiConstants.boolTrueVal, KpiConstants.boolTrueVal)
=======
<<<<<<< HEAD
    val ageFilterDf = UtilFunctions.ageFilter(commonFilterDf, KpiConstants.dobColName, year, KpiConstants.age9val, KpiConstants.age13Val, KpiConstants.boolTrueVal, KpiConstants.boolTrueVal)
=======
...
>>>>>>> a8f5a41106c0275f1944059736563fc2bdc80753
    ...
    val joinForDinominatorExcl = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.ImahpvMeasureId, KpiConstants.cisImamenDinoExclValueSet, KpiConstants.primaryDiagnosisCodeSystem)
    ...
    val measurement = UtilFunctions.mesurementYearFilter(hedisJoinedForImahpvScreeningDf, KpiConstants.startDateColName, year, KpiConstants.measurementYearLower, KpiConstants.measurementOneyearUpper)
    ...
    val cisImahpvJoinDf = ageFilterJoinNumeratorDf.as("df1").join(factClaimDf.as("df2"), $"df1.member_sk" === $"df2.member_sk", "inner").select("df2.member_sk", "df2.start_date_sk")

    /* Filtering members with At least three IMAHPV vaccinations */

    val cisImahpvCountDf = cisImahpvJoinDf.groupBy("member_sk").agg(count("start_date_sk").alias("count1")).filter($"count1".>=(3)).select(KpiConstants.memberskColName)

    val cisImahpvNumeratorDf = cisImahpvCountDf.intersect(finalDinominatorDf)

    cisImahpvNumeratorDf.show()


    /*common output creation(data to fact_gaps_in_hedis table)*/
    val numeratorReasonValueSet = KpiConstants.cisImahpvValueSet
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.ImahpvMeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinominatorExcl, cisImahpvNumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
