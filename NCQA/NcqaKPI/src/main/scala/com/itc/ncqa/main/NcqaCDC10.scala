...
object NcqaCDC10 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dinominatorExclusionDf = hospiceDf.union(fralityAndAdvIlDfAndAbove65Df).union(dinoExclusion2Df)
    /*Dinominator after Dinominator Exclusion*/
    val dinominatorAfterExclDf = dinominatorForKpiCalDf.except(dinominatorExclusionDf)
    //</editor-fold>

    //<editor-fold desc="Numerator Calculation">

    //<editor-fold desc="Numerator (most recent HbA1c test during the measurement year)">

    val hba1cValList = List(KpiConstants.hba1cTestVal)
    val hba1cCodeVal = List(KpiConstants.cptCodeVal, KpiConstants.loincCodeVal)
    val joinedForHba1cDf = UtilFunctions.factClaimRefHedisJoinFunction(spark,factClaimDf,refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType,KpiConstants.cdcMeasureId,hba1cValList,hba1cCodeVal)
    val measurForHba1cDf = UtilFunctions.measurementYearFilter(joinedForHba1cDf,KpiConstants.startDateColName,year, KpiConstants.measurement0Val, KpiConstants.measurement1Val).select(KpiConstants.memberskColName,KpiConstants.startDateColName)
    val mostrecentHba1cDf = UtilFunctions.mostRececntHba1cTest(measurForHba1cDf,KpiConstants.startDateColName,year).select(KpiConstants.memberskColName)
    //</editor-fold>

    //<editor-fold desc="Numerator (HbA1c level is <8.0%)">

    val hba1cLt8ValList = List(KpiConstants.hba1cLtSevenVal)
    val hba1cLt8CodeVal = List(KpiConstants.cptCodeVal)
    val joinedForHba1c8Df = UtilFunctions.factClaimRefHedisJoinFunction(spark,factClaimDf,refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType,KpiConstants.cdcMeasureId,hba1cLt8ValList,hba1cLt8CodeVal)
    val measurForHba1c8Df = UtilFunctions.measurementYearFilter(joinedForHba1c8Df,KpiConstants.startDateColName,year, KpiConstants.measurement0Val, KpiConstants.measurement1Val).select(KpiConstants.memberskColName)
    //</editor-fold>

    val hba1cLessThan8NumeratorDf = mostrecentHba1cDf.intersect(measurForHba1c8Df)
    val numeratorDf = hba1cLessThan8NumeratorDf.intersect(dinominatorAfterExclDf)
    //numeratorDf.show()
    //</editor-fold>

    //<editor-fold desc="Output creation and Store the o/p to Fact_Gaps_In_Heids Table">

    val numeratorReasonValueSet = hba1cValList ::: hba1cLt8ValList
