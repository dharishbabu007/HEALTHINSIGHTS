...
/*
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NcqaWCC2B")
    ...
    /*Age 12–17 (Nutrition Counseling)*/
    ...
    val hedisJoinedForDinominator = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.wcc2bMeasureId, KpiConstants.abavalueSetForDinominator, KpiConstants.abscodeSystemForDinominator)
    ...
    val joinForDinominatorExcl = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.wcc2bMeasureId, KpiConstants.abavaluesetForDinExcl, KpiConstants.primaryDiagnosisCodeSystem)
    ...
    val joinForNumeratorForNutrition1 = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.wcc2bMeasureId, KpiConstants.wcc1bNutritionValueSet, KpiConstants.primaryDiagnosisCodeSystem)
    ...
    /*Numerator1 Calculation (Nutrition Counseling  Value Set ) for primaryDiagnosisCodeSystem */
    val joinForNumeratorForNutrition2 = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.wcc2bMeasureId, KpiConstants.wcc1bNutritionValueSet, KpiConstants.wcc1bNutritionCodeSystem)
    ...
    val wcc2bnumeratorFinalDf = numeratorNutritionDf.intersect(finalDinominatorDf).select(KpiConstants.memberskColName).distinct()

    wcc2bnumeratorFinalDf.show()
    ...
    val sourceAndMsrIdList = List(data_source,KpiConstants.wcc2bMeasureId)
    val numExclDf = spark.emptyDataFrame
    val outFormatDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorForOutput, dinominatorExcl, wcc2bnumeratorFinalDf, numExclDf, listForOutput, sourceAndMsrIdList)
