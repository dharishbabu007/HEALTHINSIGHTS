...
object NcqaIMACMB2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQAIMACMB2")
...
>>>>>>> a8f5a41106c0275f1944059736563fc2bdc80753
    ...
    val joinForDinominatorExcl = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.Imacmb2MeasureId, KpiConstants.cisImamenDinoExclValueSet, KpiConstants.primaryDiagnosisCodeSystem)
...
>>>>>>> a8f5a41106c0275f1944059736563fc2bdc80753
    ...
    val cisImahpvJoinDf = ageFilterJoinNumeratorDf2.as("df1").join(factClaimDf.as("df2"), $"df1.member_sk" === $"df2.member_sk", "inner").select("df2.member_sk", "df2.start_date_sk")
    ...
    val cisImacmb2NumeratorDf = cisImamenCountDf.union(cisImatdCountDf).union(cisImahpvCountDf).select(KpiConstants.memberskColName)

    val cisImacmb2FinalNumeratorDf = cisImacmb2NumeratorDf.intersect(finalDinominatorDf)

    cisImacmb2FinalNumeratorDf.show()


    /*common output creation(data to fact_gaps_in_hedis table)*/
    val numeratorReasonValueSet = KpiConstants.cisImamenValueSet ::: KpiConstants.cisImatdValueSet ::: KpiConstants.cisImahpvValueSet
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.Imacmb2MeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinominatorExcl, cisImacmb2NumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
