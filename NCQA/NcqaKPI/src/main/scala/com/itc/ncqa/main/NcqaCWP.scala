...
object NcqaCWP {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQACWP")
    ...
    val initialJoinedDf = UtilFunctions.joinForCommonFilterFunction(spark, dimMemberDf, factClaimDf, factMembershipDf, dimLocationDf, refLobDf, dimFacilityDf, lob_name, KpiConstants.cwpMeasureTitle)
    ...
    val ageFilterDf = newDf2.filter((datediff(newDf2.col("curr_date"), newDf2.col(KpiConstants.dobColName)) / 365.25).<=(KpiConstants.age18Val.toInt) && (datediff(newDf2.col("last_Year_date"), newDf2.col(KpiConstants.dobColName)) / 4383).>=(KpiConstants.age3Val.toInt))
    ...
    val cwpObserEdValSet = List(KpiConstants.observationVal,KpiConstants.edVal)
    val cwpObserEdCodeSystem = List(KpiConstants.cptCodeVal,KpiConstants.ubrevCodeVal)
    val joinedForObserEdDf = UtilFunctions.factClaimRefHedisJoinFunction(spark,factClaimDf,refHedisDf,KpiConstants.proceedureCodeColName,KpiConstants.innerJoinType,KpiConstants.cwpMeasureId,cwpObserEdValSet,cwpObserEdCodeSystem)
    ...
    val joinedForInPatDf = UtilFunctions.factClaimRefHedisJoinFunction(spark,factClaimDf,refHedisDf,KpiConstants.proceedureCodeColName,KpiConstants.innerJoinType,KpiConstants.cwpMeasureId,inPatValSet,inPatCodeSystem)
    ...
    val joinedForOutPatDf = UtilFunctions.factClaimRefHedisJoinFunction(spark,factClaimDf,refHedisDf,KpiConstants.proceedureCodeColName,KpiConstants.innerJoinType,KpiConstants.cwpMeasureId,outPatValSet,outPatCodeSystem)
    ...
    /*CWP (Pharyngitis Value Set) AS Primary Diagnosis*/
    val cwpDiaValSet = List(KpiConstants.pharyngitisVal)
    val joinedForCwpAsDiaDf = UtilFunctions.factClaimRefHedisJoinFunction(spark,factClaimDf,refHedisDf,KpiConstants.primaryDiagnosisColname ,KpiConstants.innerJoinType,KpiConstants.cwpMeasureId,cwpDiaValSet,primaryDiagnosisCodeSystem)
    val msrFilCwpisAsDiagDf = UtilFunctions.dateBetweenFilter(joinedForCwpAsDiaDf ,KpiConstants.startDateColName,firstIntakeDate,secondIntakeDate)

    /* Outpatient Value Set, Observation Value Set,ED visit with only diagnoses of Pharyngitis Valueset */
    val step1Df = (msrFilForOutPatDf.select(KpiConstants.memberskColName).union(edObserWInPatDf)).intersect(msrFilCwpisAsDiagDf.select(KpiConstants.memberskColName))
    //</editor-fold>

    //<editor-fold desc=" Step2">

    /* Identify Episode Date - Ed and Outpatient Valueset dcwpng Intake period with CWP Value Set */
    ...
    /* Determine if antibiotics (CWP Antibiotic Medications List) were dispensed for any of the Episode Dates */
    val ref_medvaluesetDf = DataLoadFunctions.referDataLoadFromTragetModel(spark, KpiConstants.dbName, KpiConstants.refmedValueSetTblName)
    val cwpAntiMedListVal = List(KpiConstants.cwpAntibioticMedicationListsVal)
    val cwpAntiListDf = UtilFunctions.factRxClaimRefMedValueSetJoinFunction(spark,factClaimDf,ref_medvaluesetDf,KpiConstants.cwpMeasureId,cwpAntiMedListVal)
    ...
    val step3ConditionDf = step2Df.as("df1").join(cwpAntiListDf.as("df2"), step2Df.col(KpiConstants.memberskColName) === cwpAntiListDf.col(KpiConstants.memberskColName),KpiConstants.innerJoinType)
                                  .filter(datediff(step2Df.col(KpiConstants.startDateColName),cwpAntiListDf.col(KpiConstants.rxStartDateColName)).>=(KpiConstants.days3))
    ...
    val step4ConditionDf = step3Df.as("df1").join(cwpAntiListDf.as("df2"), step3Df.col(KpiConstants.memberskColName) === cwpAntiListDf.col(KpiConstants.memberskColName),KpiConstants.innerJoinType)
                         .filter(datediff(cwpAntiListDf.col(KpiConstants.startDateColName),step3Df.col(KpiConstants.startDateColName)).<=(KpiConstants.days30))
    ...
    /* Group A Strep Tests Value Set on or three days after the IESD */
    ...
    val numCwpDf = joinedForCwpNumDf.as("df1").join(step6Df.as("df2"), $"df1.${KpiConstants.memberskColName}" === $"df2.${KpiConstants.memberskColName}", KpiConstants.innerJoinType)
      .filter((datediff($"df1.${KpiConstants.startDateColName}", $"df2.${KpiConstants.iesdDateColName}").>= ((KpiConstants.days3))) &&
        (datediff($"df1.${KpiConstants.startDateColName}", $"df2.${KpiConstants.iesdDateColName}").<= ((-3))))
    ...
    val numeratorValueSet = cwpNumVal
