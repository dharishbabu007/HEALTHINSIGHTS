...
object NcqaCAP2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NcqaCAP2")
    ...
    //commonFilterDf.select(KpiConstants.memberskColName,KpiConstants.dobColName).show(100)
    ...
    val ageFilterDf = newDf2.filter((datediff(newDf2.col("curr_date"), newDf2.col(KpiConstants.dobColName)) / 30).>=(25) && (datediff(newDf2.col("curr_date"), newDf2.col(KpiConstants.dobColName)) / 30).<=(72)).drop("curr_date")
    //ageFilterDf.select(KpiConstants.memberskColName,KpiConstants.dobColName).show(100)
    ...
    val measurementForAmbVisAsDiagDf = UtilFunctions.mesurementYearFilter(joinedForAmbVisAsDiagDf,KpiConstants.startDateColName,year,KpiConstants.measurementYearLower,KpiConstants.measurementOneyearUpper).select(KpiConstants.memberskColName)
    ...
    val measurementForAmbVisAsProDf = UtilFunctions.mesurementYearFilter(joinedForAmbVisAsProDf,KpiConstants.startDateColName,year,KpiConstants.measurementYearLower,KpiConstants.measurementOneyearUpper).select(KpiConstants.memberskColName)
