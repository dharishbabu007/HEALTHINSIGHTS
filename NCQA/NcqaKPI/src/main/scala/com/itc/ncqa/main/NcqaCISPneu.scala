...
object NcqaCISPneu {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQACISPNEU")
    ...
    /*Numerator1 Calculation (Pneu screening or monitoring test)*/


    val hedisJoinedForPneuScreeningDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cisPneuMeasureId, KpiConstants.cisPneuValueSet, KpiConstants.cisPneuCodeSystem)

    val measurement = UtilFunctions.mesurementYearFilter(hedisJoinedForPneuScreeningDf, KpiConstants.startDateColName, year, KpiConstants.measurementYearLower, KpiConstants.measuremetTwoYearUpper)
    ...
    val cisPneuJoinDf = dayFilterCondionNumeratorDf.as("df1").join(factClaimDf.as("df2"), $"df1.member_sk" === $"df2.member_sk", "inner").select("df2.member_sk", "df2.start_date_sk")

    /* Filtering members with At least three Pneu vaccinations */

    val cisPneuCountDf = cisPneuJoinDf.groupBy("member_sk").agg(count("start_date_sk").alias("count1")).filter($"count1".>=(3)).select("df2.member_sk")

    val cisPneuNumeratorDf = cisPneuCountDf.intersect(dinominatorAfterExclusionDf)

    cisPneuNumeratorDf.show()

    /*common output creation(data to fact_gaps_in_hedis table)*/
    val numeratorReasonValueSet = KpiConstants.cisPneuValueSet
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.cisPneuMeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinoExclDf, cisPneuNumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
