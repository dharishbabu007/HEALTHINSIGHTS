...
object NcqaCDC9 {
  ...
  /*def main(args: Array[String]): Unit = {
    ...
    /*CDC9 dinominator for kpi calculation*/
    val cdc9DinominatorForKpiCal = dinominatorDf.except(dinominatorExclusionDf)


    /*Numerator Calculation starts*/
    /*Numerator1 (Systolic Less Than 140 Value Set)*/
    val hedisJoinedForSystolicDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark,dimMemberDf,factClaimDf,refHedisDf,"procedure_code","inner",KpiConstants.cdcMeasureId,KpiConstants.cdc9SystolicValueSet,KpiConstants.cdc9SystolicAndDiastolicCodeSystem)
    val measurementForSystolicDf = UtilFunctions.mesurementYearFilter(hedisJoinedForSystolicDf,"start_date",year,0,365).select("member_sk","start_date").select("member_sk")


    /*Numerator2 (Diastolic)*/
    val hedisJoinedForDiastolicDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark,dimMemberDf,factClaimDf,refHedisDf,"procedure_code","inner",KpiConstants.cdcMeasureId,KpiConstants.cdc9DiastolicValueSet,KpiConstants.cdc9SystolicAndDiastolicCodeSystem)
    val measurementForDiastolicDf = UtilFunctions.mesurementYearFilter(hedisJoinedForDiastolicDf,"start_date",year,0,365).select("member_sk","start_date").select("member_sk")

    /*Numerator (Join of Numerator1 and Numerator2)*/
    val unionNumeratorDf = measurementForSystolicDf.union(measurementForDiastolicDf).distinct()

    val cdc9NumeratorDf = unionNumeratorDf.intersect(cdc9DinominatorForKpiCal)
   ...
   /* val lobdetailsData = dinominatorDf.as("df1").join(factMembershipDf.as("df2"),$"df1.member_sk" === $"df2.member_sk").select("df1.member_sk","df2.lob_id")
    ...
    val formattedOutPutDf = UtilFunctions.outputDfCreation(spark,dataDf,dinominatorExclusionDf,cdc9NumeratorDf,dimMemberDf,KpiConstants.cdcMeasureId)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQACDC9")
    val spark = SparkSession.builder().config(conf).enableHiveSupport().getOrCreate()
    val year = args(0)
    val lob_name = args(1)
    ...
    val cdc9DinominatorForKpiCalDf = dinoMemberSkDf.except(dinominatorExclusionDf)


    /*Numerator Calculation starts*/
    /*Numerator1 (Systolic Less Than 140 Value Set)*/
    val hedisJoinedForSystolicDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark,dimMemberDf,factClaimDf,refHedisDf,KpiConstants.proceedureCodeColName,KpiConstants.innerJoinType,KpiConstants.cdcMeasureId,KpiConstants.cdc9SystolicValueSet,KpiConstants.cdc9SystolicAndDiastolicCodeSystem)
    val measurementForSystolicDf = UtilFunctions.mesurementYearFilter(hedisJoinedForSystolicDf,"start_date",year,KpiConstants.measurementYearLower,KpiConstants.measurementOneyearUpper).select(KpiConstants.memberskColName)


    /*Numerator2 (Diastolic)*/
    val hedisJoinedForDiastolicDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark,dimMemberDf,factClaimDf,refHedisDf,"procedure_code","inner",KpiConstants.cdcMeasureId,KpiConstants.cdc9DiastolicValueSet,KpiConstants.cdc9SystolicAndDiastolicCodeSystem)
    val measurementForDiastolicDf = UtilFunctions.mesurementYearFilter(hedisJoinedForDiastolicDf,"start_date",year,KpiConstants.measurementYearLower,KpiConstants.measurementOneyearUpper).select(KpiConstants.memberskColName)

    /*Numerator (Join of Numerator1 and Numerator2)*/
    val unionNumeratorDf = measurementForSystolicDf.union(measurementForDiastolicDf)

    val cdc9NumeratorDf = unionNumeratorDf.intersect(cdc9DinominatorForKpiCalDf)
    cdc9NumeratorDf.show()
    /*common output creation(data to fact_gaps_in_hedis table)*/
    val numeratorReasonValueSet = KpiConstants.cdc9SystolicValueSet ::: KpiConstants.cdc9DiastolicValueSet
    ...
    val sourceAndMsrList = List(data_source,KpiConstants.cdc9MeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinominatorExclusionDf, cdc9NumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
