...
object NcqaCDC9 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //<editor-fold desc="Numerator1 (Systolic Less Than 140 Value Set)">

    val systolicValList = List(KpiConstants.systolicLt140Val)
    val systolicCodeVal = List(KpiConstants.cptCodeVal)
    val joinedForSystolicDf = UtilFunctions.factClaimRefHedisJoinFunction(spark,factClaimDf,refHedisDf,KpiConstants.proceedureCodeColName,KpiConstants.innerJoinType,KpiConstants.cdcMeasureId,systolicValList,systolicCodeVal)
    val measurForSystolicDf = UtilFunctions.measurementYearFilter(joinedForSystolicDf,KpiConstants.startDateColName,year,KpiConstants.measurement0Val,KpiConstants.measurement1Val).select(KpiConstants.memberskColName)
    //</editor-fold>

    //<editor-fold desc="Numerator2 (Diastolic Valueset)">

    val diastolicValList = List(KpiConstants.diastolicLt80Val)
    val diastolicCodeVal = List(KpiConstants.cptCodeVal)
    val joinedForDiastolicDf = UtilFunctions.factClaimRefHedisJoinFunction(spark,factClaimDf,refHedisDf,KpiConstants.proceedureCodeColName,KpiConstants.innerJoinType,KpiConstants.cdcMeasureId,diastolicValList,diastolicCodeVal)
    val measurForDiastolicDf = UtilFunctions.measurementYearFilter(joinedForDiastolicDf,KpiConstants.startDateColName,year,KpiConstants.measurement0Val,KpiConstants.measurement1Val).select(KpiConstants.memberskColName)
    //</editor-fold>

    /*Numerator (Join of Numerator1 and Numerator2)*/
    val unionNumeratorDf = measurForSystolicDf.union(measurForDiastolicDf)
    val numeratorDf = unionNumeratorDf.intersect(dinominatorAfterExclDf)
    //numeratorDf.show()
    //</editor-fold>

    //<editor-fold desc="Output creation and Store the o/p to Fact_Gaps_In_Heids Table">

    val numeratorReasonValueSet = systolicValList ::: diastolicValList
