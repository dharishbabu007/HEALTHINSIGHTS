...
>>>>>>> a8f5a41106c0275f1944059736563fc2bdc80753
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQAAAP1")
...
<<<<<<< HEAD
    val ageFilterDf = UtilFunctions.ageFilter(commonFilterDf, KpiConstants.dobColName, year, KpiConstants.age20Val, KpiConstants.age44Val, KpiConstants.boolTrueVal, KpiConstants.boolTrueVal)
=======
<<<<<<< HEAD
    val ageFilterDf = UtilFunctions.ageFilter(commonFilterDf, KpiConstants.dobColName, year, KpiConstants.age20Val, KpiConstants.age44val, KpiConstants.boolTrueVal, KpiConstants.boolTrueVal)
=======
...
>>>>>>> a8f5a41106c0275f1944059736563fc2bdc80753
    ...
    val joinForNumeratorForaap11 = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.aap1MeasureId, KpiConstants.aapValueSet, KpiConstants.primaryDiagnosisCodeSystem)
    val measurementNumeratoraap11 = UtilFunctions.mesurementYearFilter(joinForNumeratorForaap11, KpiConstants.startDateColName, year, KpiConstants.measurementYearLower, measurementYearUpper)
    val numeratoraap1Df1 = ageFilterDf.as("df1").join(measurementNumeratoraap11.as("df2"), ageFilterDf.col(KpiConstants.memberskColName) === measurementNumeratoraap11.col(KpiConstants.memberskColName), KpiConstants.innerJoinType).select("df1.member_sk")

    /*Numerator1 Calculation  for proceedureCodeSystem */
    val joinForNumeratorForaap12 = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.aap1MeasureId, KpiConstants.aapValueSet, KpiConstants.aapCodeSystem)
    val measurementNumeratoraap12 = UtilFunctions.mesurementYearFilter(joinForNumeratorForaap12, KpiConstants.startDateColName, year, KpiConstants.measurementYearLower, measurementYearUpper)
    val numeratoraap1Df2 = ageFilterDf.as("df1").join(measurementNumeratoraap12.as("df2"), ageFilterDf.col(KpiConstants.memberskColName) === measurementNumeratoraap12.col(KpiConstants.memberskColName), KpiConstants.innerJoinType).select("df1.member_sk")

    val numeratoraap1Df = numeratoraap1Df1.union(numeratoraap1Df2)

    /*Final Numerator(Elements who are present in dinominator and numerator)*/
    val aap1numeratorFinalDf = numeratoraap1Df.intersect(dinominatorAfterExclusionDf).select(KpiConstants.memberskColName).distinct()

    aap1numeratorFinalDf.show()
    ...
    val sourceAndMsrIdList = List(data_source,KpiConstants.aap1MeasureId)
    val numExclDf = spark.emptyDataFrame
    val outFormatDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, hospiceDf, aap1numeratorFinalDf, numExclDf, listForOutput, sourceAndMsrIdList)
