...
object NcqaCISMmr {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQAMmr")
    ...
    /*Numerator1 Calculation (Mmr screening or monitoring test)*/

    val hedisJoinedForMmrProcCodeScreeningDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cisMmrMeasureId, KpiConstants.cisMmrValueSet, KpiConstants.cisMmrCodeSystem)
    val hedisJoinedForMmrPrimaryScreeningDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.cisMmrMeasureId, KpiConstants.cisMmrValueSet, KpiConstants.primaryDiagnosisCodeSystem)

    val hedisJoinedForMmrScreeningDf = hedisJoinedForMmrProcCodeScreeningDf.union(hedisJoinedForMmrPrimaryScreeningDf)

    val measurement = UtilFunctions.mesurementYearFilter(hedisJoinedForMmrScreeningDf, KpiConstants.startDateColName, year, KpiConstants.measurementYearLower, KpiConstants.measuremetTwoYearUpper)
    ...
    val cisMmrJoinDf = dayFilterCondionNumeratorDf.as("df1").join(factClaimDf.as("df2"), $"df1.member_sk" === $"df2.member_sk", "inner").select("df2.member_sk", "df2.start_date_sk")

    /* Filtering members with At least three Mmr vaccinations */

    val cisMmrCountDf = cisMmrJoinDf.groupBy("member_sk").agg(count("start_date_sk").alias("count1")).filter($"count1".>=(1)).select("df2.member_sk")

    val cisMmrNumeratorDf = cisMmrCountDf.intersect(dinominatorAfterExclusionDf)


    /*common output creation(data to fact_gaps_in_hedis table)*/
    val numeratorReasonValueSet = KpiConstants.cisMmrValueSet
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.cisMmrMeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinoExclDf, cisMmrNumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
