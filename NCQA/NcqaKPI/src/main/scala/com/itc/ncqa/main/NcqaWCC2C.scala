...
/*
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NcqaWCC2C")
    ...
    val hedisJoinedForDinominator = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.wcc2cMeasureId, KpiConstants.abavalueSetForDinominator, KpiConstants.abscodeSystemForDinominator)
    ...
    val joinForDinominatorExcl = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.wcc2cMeasureId, KpiConstants.abavaluesetForDinExcl, KpiConstants.primaryDiagnosisCodeSystem)
    ...
    /*Numerator1 Calculation (Physical Activity  Value Set )*/

    /*Numerator1 Calculation (Physical Activity  Value Set ) for primaryDiagnosisCodeSystem */
    val joinForNumeratorForPhysical1 = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.wcc2cMeasureId, KpiConstants.wcc1cPhysicalValueSet, KpiConstants.primaryDiagnosisCodeSystem)
    val measurementNumeratorPhysical1 = UtilFunctions.mesurementYearFilter(joinForNumeratorForPhysical1, KpiConstants.startDateColName, year, KpiConstants.measurementYearLower, KpiConstants.measurementOneyearUpper)
    val numeratorPhysicalDf1 = ageFilterDf.as("df1").join(measurementNumeratorPhysical1.as("df2"), ageFilterDf.col(KpiConstants.memberskColName) === measurementNumeratorPhysical1.col(KpiConstants.memberskColName), KpiConstants.innerJoinType).select("df1.member_sk")

    /*Numerator1 Calculation (Physical Activity  Value Set ) for primaryDiagnosisCodeSystem */
    val joinForNumeratorForPhysical2 = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.wcc2cMeasureId, KpiConstants.wcc1cPhysicalValueSet, KpiConstants.wcc1cPhysicalCodeSystem)
    val measurementNumeratorPhysical2 = UtilFunctions.mesurementYearFilter(joinForNumeratorForPhysical2, KpiConstants.startDateColName, year, KpiConstants.measurementYearLower, KpiConstants.measurementOneyearUpper)
    val numeratorPhysicalDf2 = ageFilterDf.as("df1").join(measurementNumeratorPhysical2.as("df2"), ageFilterDf.col(KpiConstants.memberskColName) === measurementNumeratorPhysical2.col(KpiConstants.memberskColName), KpiConstants.innerJoinType).select("df1.member_sk")

    val numeratorPhysicalDf = numeratorPhysicalDf1.union(numeratorPhysicalDf2)

    /*Final Numerator(Elements who are present in dinominator and numerator)*/
    val wcc2cnumeratorFinalDf = numeratorPhysicalDf.intersect(finalDinominatorDf).select(KpiConstants.memberskColName).distinct()

    wcc2cnumeratorFinalDf.show()

    /*Common output format (data to fact_hedis_gaps_in_care)*/
    val numeratorValueSet = KpiConstants.wcc1cPhysicalValueSet
    ...
    val sourceAndMsrIdList = List(data_source,KpiConstants.wcc2cMeasureId)
    val numExclDf = spark.emptyDataFrame
    val outFormatDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorForOutput, dinominatorExcl, wcc2cnumeratorFinalDf, numExclDf, listForOutput, sourceAndMsrIdList)
