...
object NcqaW154 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQAW154")
    ...
    val hedisJoinedForWellCareAsPrDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.w154MeasureId, KpiConstants.w15ValueSetForNumerator, KpiConstants.w15CodeSystemForNum)
    ...
    val hedisJoinedForWellCareAsDiagDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.w154MeasureId, KpiConstants.w15ValueSetForNumerator, KpiConstants.primaryDiagnosisCodeSystem)
    ...
    val w154NumeratorGeneral = measurementForWellCareAsPrDf.union(measurementForWellCareAsDiagDf)

    /*Start_date_sk for members who have pcp equal to 'Y'*/


    val w154WellCareJoinDf = dimProviderDf.as("df1").join(factClaimDf.as("df2"), ($"df1.provider_sk" === $"df2.provider_sk"), "inner").filter(($"df1.pcp" === KpiConstants.yesVal)).select("df2.member_sk", "df2.start_date_sk").distinct()

    val w154JoinNumerator = w154NumeratorGeneral.as("df1").join(w154WellCareJoinDf.as("df2"), ($"df1.member_sk" === $"df2.member_sk"), joinType = "inner").select("df2.member_sk", "df2.start_date_sk")

    val w154WellCareCountZeroDf = w154JoinNumerator.groupBy("member_sk").agg(count("start_date_sk").alias("count1")).filter($"count1".===(4)).select(KpiConstants.memberskColName)

    val w154NumeratorDf = w154WellCareCountZeroDf.intersect(dinominatorAfterExclusionDf)
    w154NumeratorDf.show()
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.w154MeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinominatorAfterExclusionDf, w154NumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
