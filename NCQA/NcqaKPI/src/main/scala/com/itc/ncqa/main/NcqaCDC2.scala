...
object NcqaCDC2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val hba1cgt9ValList = List(KpiConstants.hba1cGtNineVal)
    val hba1cgt9CodeSystem = List(KpiConstants.cptcat2CodeVal)
    val hedisJoinedForhba1cgt9Df = UtilFunctions.factClaimRefHedisJoinFunction(spark, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cdcMeasureId, hba1cgt9ValList, hba1cgt9CodeSystem)
    val measurForhba1cgt9Df = UtilFunctions.measurementYearFilter(hedisJoinedForhba1cgt9Df, KpiConstants.startDateColName, year, KpiConstants.measurement0Val, KpiConstants.measurement1Val).select(KpiConstants.memberskColName)

    /*CDC2 Numerator Calculation */
    val numeratorIntersectionDf = measurementForHba1cDf.intersect(measurForhba1cgt9Df)
    ...
    val numeratorReasonValueSet = hba1cTestValList ::: hba1cgt9ValList
