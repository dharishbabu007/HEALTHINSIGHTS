...
object NcqaCDC2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val measurementDiabetesExclDf = UtilFunctions.mesurementYearFilter(hedisJoinedForDiabetesExclDf,"start_date",year,0,730).select("member_sk")
    ...
    val cdc2DinominatorForKpiCalDf = dinominatorDf.except(dinominatorExclusionDf)
    ...
    val measurementForNumerator = UtilFunctions.mesurementYearFilter(hedisJoinedForNumeratorDf,"start_date",year,0,365).select("member_sk").distinct()
    ...
    val formattedOutPutDf = UtilFunctions.outputDfCreation(spark,dataDf,dinominatorExclusionDf,cdc2NumeratorDf,dimMemberDf,KpiConstants.cdcMeasureId)
