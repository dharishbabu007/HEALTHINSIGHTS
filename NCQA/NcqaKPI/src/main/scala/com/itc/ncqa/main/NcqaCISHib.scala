...
object NcqaCISHib {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQAHiB")
    ...
    /*Numerator1 Calculation (HiB screening or monitoring test)*/

    val hedisJoinedForHiBScreeningDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cisHiBMeasureId, KpiConstants.cisHiBValueSet, KpiConstants.cisHiBCodeSystem)
    val measurement = UtilFunctions.mesurementYearFilter(hedisJoinedForHiBScreeningDf, KpiConstants.startDateColName, year, KpiConstants.measurementYearLower, KpiConstants.measuremetTwoYearUpper)
    ...
    val cisHiBJoinDf = dayFilter42NumeratorDf.as("df1").join(factClaimDf.as("df2"), $"df1.member_sk" === $"df2.member_sk", "inner").select("df2.member_sk", "df2.start_date_sk")

    /* Filtering members with At least three HiB vaccinations */

    val cisHiBCountDf = cisHiBJoinDf.groupBy("member_sk").agg(count("start_date_sk").alias("count1")).filter($"count1".>=(3)).select("df2.member_sk")

    val cisHiBNumeratorDf = cisHiBCountDf.intersect(dinominatorAfterExclusionDf)


    /*common output creation(data to fact_gaps_in_hedis table)*/
    val numeratorReasonValueSet = KpiConstants.cisHiBValueSet
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.cisHiBMeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinoExclDf, cisHiBNumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
