...
object NcqaIMAMEN {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQAIMAMEN")
    ...
    val joinForDinominatorExcl = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.ImamenMeasureId, KpiConstants.cisImamenDinoExclValueSet, KpiConstants.primaryDiagnosisCodeSystem)
    ...
    val measurement = UtilFunctions.mesurementYearFilter(hedisJoinedForImamenScreeningDf, KpiConstants.startDateColName, year, KpiConstants.measurementYearLower, KpiConstants.measurementOneyearUpper)
    ...
    val cisImamenJoinDf = ageFilterJoinNumeratorDf.as("df1").join(factClaimDf.as("df2"), $"df1.member_sk" === $"df2.member_sk", "inner").select("df2.member_sk", "df2.start_date_sk")
    ...
    val cisImamenNumeratorDf = cisImamenCountDf.intersect(finalDinominatorDf)

    cisImamenNumeratorDf.show()


    /*common output creation(data to fact_gaps_in_hedis table)*/
    val numeratorReasonValueSet = KpiConstants.cisImamenValueSet
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.ImamenMeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinominatorExcl, cisImamenNumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
