...
object NcqaCISIpv {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /*Numerator1 Calculation (IPV screening or monitoring test)*/

    val hedisJoinedForIpvScreeningDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.cisIpvMeasureId, KpiConstants.cisIpvValueSet, KpiConstants.cisIpvCodeSystem)
    val measurement = UtilFunctions.mesurementYearFilter(hedisJoinedForIpvScreeningDf, KpiConstants.startDateColName, year, KpiConstants.measurementYearLower, KpiConstants.measuremetTwoYearUpper)
    ...
    val cisIpvJoinDf = dayFilter42NumeratorDf.as("df1").join(factClaimDf.as("df2"), $"df1.member_sk" === $"df2.member_sk", "inner").select("df2.member_sk", "df2.start_date_sk")

    /* Filtering members with At least three IPV vaccinations */

    val cisIpvCountDf = cisIpvJoinDf.groupBy("member_sk").agg(count("start_date_sk").alias("count1")).filter($"count1".>=(3)).select("df2.member_sk")

    val cisIpvNumeratorDf = cisIpvCountDf.intersect(dinominatorAfterExclusionDf)

    cisIpvNumeratorDf.show()


    /*common output creation(data to fact_gaps_in_hedis table)*/
    val numeratorReasonValueSet = KpiConstants.cisIpvValueSet
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.cisIpvMeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinoExclDf, cisIpvNumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
