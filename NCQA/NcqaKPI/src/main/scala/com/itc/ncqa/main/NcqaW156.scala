...
object NcqaW156 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQAW156")
    ...
    val hedisJoinedForWellCareAsPrDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.w156MeasureId, KpiConstants.w15ValueSetForNumerator, KpiConstants.w15CodeSystemForNum)
    ...
    val hedisJoinedForWellCareAsDiagDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.w156MeasureId, KpiConstants.w15ValueSetForNumerator, KpiConstants.primaryDiagnosisCodeSystem)
    ...
    val w156NumeratorGeneral = measurementForWellCareAsPrDf.union(measurementForWellCareAsDiagDf)

    /*Start_date_sk for members who have pcp equal to 'Y'*/


    val w156WellCareJoinDf = dimProviderDf.as("df1").join(factClaimDf.as("df2"), ($"df1.provider_sk" === $"df2.provider_sk"), "inner").filter(($"df1.pcp" === KpiConstants.yesVal)).select("df2.member_sk", "df2.start_date_sk").distinct()

    val w156JoinNumerator = w156NumeratorGeneral.as("df1").join(w156WellCareJoinDf.as("df2"), ($"df1.member_sk" === $"df2.member_sk"), joinType = "inner").select("df2.member_sk", "df2.start_date_sk")

    val w156WellCareCountZeroDf = w156JoinNumerator.groupBy("member_sk").agg(count("start_date_sk").alias("count1")).filter($"count1".===(6)).select(KpiConstants.memberskColName)

    val w156NumeratorDf = w156WellCareCountZeroDf.intersect(dinominatorAfterExclusionDf)
    w156NumeratorDf.show()
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.w156MeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinominatorAfterExclusionDf, w156NumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
