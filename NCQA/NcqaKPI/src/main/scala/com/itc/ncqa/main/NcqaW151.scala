...
object NcqaW151 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("NCQAW151")
    ...
    val hedisJoinedForWellCareAsPrDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.proceedureCodeColName, KpiConstants.innerJoinType, KpiConstants.w151MeasureId, KpiConstants.w15ValueSetForNumerator, KpiConstants.w15CodeSystemForNum)
    ...
    val hedisJoinedForWellCareAsDiagDf = UtilFunctions.dimMemberFactClaimHedisJoinFunction(spark, dimMemberDf, factClaimDf, refHedisDf, KpiConstants.primaryDiagnosisColname, KpiConstants.innerJoinType, KpiConstants.w151MeasureId, KpiConstants.w15ValueSetForNumerator, KpiConstants.primaryDiagnosisCodeSystem)
    ...
    val w151NumeratorGeneral = measurementForWellCareAsPrDf.union(measurementForWellCareAsDiagDf)

    /*Start_date_sk for members who have pcp equal to 'Y'*/


    val w151WellCareJoinDf = dimProviderDf.as("df1").join(factClaimDf.as("df2"), ($"df1.provider_sk" === $"df2.provider_sk"), "inner").filter(($"df1.pcp" === KpiConstants.yesVal)).select("df2.member_sk", "df2.start_date_sk").distinct()

    val w151JoinNumerator = w151NumeratorGeneral.as("df1").join(w151WellCareJoinDf.as("df2"), ($"df1.member_sk" === $"df2.member_sk"), joinType = "inner").select("df2.member_sk", "df2.start_date_sk")

    val w151WellCareCountZeroDf = w151JoinNumerator.groupBy("member_sk").agg(count("start_date_sk").alias("count1")).filter($"count1".===(1)).select(KpiConstants.memberskColName)

    val w151NumeratorDf = w151WellCareCountZeroDf.intersect(dinominatorAfterExclusionDf)
    w151NumeratorDf.show()
    ...
    val sourceAndMsrList = List(data_source, KpiConstants.w151MeasureId)

    val numExclDf = spark.emptyDataFrame
    val commonOutputFormattedDf = UtilFunctions.commonOutputDfCreation(spark, dinominatorDf, dinominatorAfterExclusionDf, w151NumeratorDf, numExclDf, outReasonValueSet, sourceAndMsrList)
